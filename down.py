...
if userInput == '1':
    ...
    if not re.match(r'[0-9]+(?:\.[0-9]+){3}', ip_add):
        ...
        userMessage = input("Message to user: ")
        downTime = input("The process will execute after X SEC: ")
        if userOption == '1':
            res = subprocess.Popen(['shutdown','-c',userMessage,'-t',downTime, '-r','-m', remote_ip_add]).communicate()[0]
            print(res)
        elif userOption == '2':
            res = subprocess.Popen(['shutdown','-c',userMessage,'-t',downTime, '-s','-m', remote_ip_add]).communicate()[0]
            print(res)
        else:
            print('Invalid input!')
